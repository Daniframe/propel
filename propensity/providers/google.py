"""Google's Gemini API, through the google-genai SDK: CLAUDE.md §4.4.
...
class GoogleProvider:
    ...
    def config(self, system: str, temperature: float, max_tokens) -> dict:
        """The system part travels as `system_instruction` here (§4.1 rule 6). The SDK takes a
        plain dict for the config, so no SDK type is needed to build it. No tools are passed, so
        the SDK's automatic function calling loop is switched off: one prompt, one request."""
